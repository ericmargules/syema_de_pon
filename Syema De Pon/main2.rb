...
class GameWindow < Gosu::Window
	...
	def initialize(width, height, fullscreen)
		...
		@tiles = Gosu::Image.load_tiles(self, "images/letters_spritesheet.png", 56, 52, false)
		@numbers = Gosu::Image.load_tiles(self, "images/numbers_spritesheet.png", 24, 28, false)
		@vert_cursor = Gosu::Image.new(self, "images/vert_cursor.png", false)
		@horz_cursor = Gosu::Image.new(self, "images/horz_cursor.png", false)
		@game = Game.new([[3, 6], [4, 6]], "horizontal")
		add_row(3)
		@game_timer = Timer.new(self)
		@last = 0
	end
  ...
  def button_down(id)
	...
	def draw
		...
		@game_timer.minutes.to_s.rjust(2, padstr="0").split("").each_with_index do |int, index|
			number = Number.new([(500 + (index * 30)), 25], int.to_i, @numbers)
			number.draw
		end
		
		@game_timer.seconds.to_s.rjust(2, padstr="0").split("").each_with_index do |int, index|
	...
	def accelerate
		case 
		when @game_timer.minutes <= 2
			if @game_timer.seconds % 30 == 0 && @game_timer.seconds != @last 
				add_row(1)
				@last = @game_timer.seconds
			end
		when @game_timer.minutes <= 3
			if @game_timer.seconds % 20 == 0 && @game_timer.seconds != @last 
				add_row(1)
				@last = @game_timer.seconds
			end
		when @game_timer.minutes <= 4
			if @game_timer.seconds % 15 == 0 && @game_timer.seconds != @last 
				add_row(1)
				@last = @game_timer.seconds
			end		
		when @game_timer.minutes <= 5
			if @game_timer.seconds % 10 == 0 && @game_timer.seconds != @last 
				add_row(1)
				@last = @game_timer.seconds		
			end
		when @game_timer.minutes >= 5
			if @game_timer.seconds % 5 == 0 && @game_timer.seconds != @last 
				add_row(1)
				@last = @game_timer.seconds		
			end
		end
